...
class TestSitTrain(TestCase):
    ...
    def test_run_with_no_exception_imbb(self):
        # Arrange
        train_data_file = os.path.join(os.path.dirname(__file__), "sample_data", "imdb_original.tsv")
    ...
    def test_run_with_no_exception_nli(self):
        # Arrange
        train_data_file = os.path.join(os.path.dirname(__file__),"sample_data", "snli_test.json")
        tempdir = tempfile.mkdtemp()
        batch = 3
        ...
        # Builder
        b = BuilderNli(train_data=train_data_file, val_data=train_data_file,
                    checkpoint_dir=tempdir, epochs=2,
                    early_stopping_patience=2, batch_size=batch,
                    max_seq_len=sequence_len, model_dir=tempdir)
        b.set_tokensior(mock_tokenisor)
